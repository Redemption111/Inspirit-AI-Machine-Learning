...
def draw_boxes(image_, boxes, labels):
    ...
    font = ImageFont.truetype(font='arial.ttf', 
                    size=np.floor(3e-2 * image_h + 0.5).astype('int32')) #originally /usr/share/fonts/truetype/liberation/LiberationMono-Regular.ttf
    ...
    for i, box in reversed(list(enumerate(boxes))):
        ...
        label_size = draw.textlength(label, font) #textsize is deprecated
        ...
        if top - label_size >= 0: #top - label_size[1] >= 0:
            text_origin = np.array([left, top - label_size]) #top - label_size[1] >= 0:
        else:
...
cap = cv2.VideoCapture(0) ## 0 is for internal cam; msi laptops can disable or enable cams
ESC = 27
while cap.isOpened():
    retval,img = cap.read()
    if retval:
